...
/**
 * <code>Transform</code> is a mathematical object representing a translation, a rotation and 
 * a scale for a scene-graph object in a 3D space.
 * <p>
...
public final class Transform implements ReadableTransform, Comparable<Transform>, Reusable {
    ...
    /**
     * The <code>Transform</code> identity &rarr; Translation: [0,0,0] | Rotation:
     * [0,0,0,1] | Scale: [1,1,1].
     */
    ...
    /**
     * Instantiates a new <code>Transform</code> with the provided translation
     * vector and identity values for the rotation and the scale.
     * <p>
     * The rotation: {0,0,0,1}. The scale: {1,1,1}.
     */
    ...
    /**
     * Instantiates a new <code>Transform</code> with the provided translation
     * vector and rotation quaternion and identity values for the scale.
     * <p>
     * The scale: {1,1,1}.
     */
    ...
    /**
     * Instantiates a new <code>Transform</code> with the provided translation
     * vector, rotation quaternion and scaling vector.
     */
    ...
    /**
     * Sets the components values of the provided {@link ReadableMatrix4f} to this 
    ...
    public Transform set(ReadableMatrix4f matrix) {
        ...
        this.rotation.set(matrix.m00(), matrix.m01(), matrix.m02(), matrix.m10(), matrix.m11(), matrix.m12(),
    ...
    /**
     * Sets the matrix of the <code>Transform</code> to the provided one. If the given matrix isn't 
     * purely rotational it will provide the scale of the transform {@link #setScale(float)} 
     * or its variants will throw an error.
     * 
     * @param rotation The rotation matrix if orthonormal, otherwise the rotation
     *                 and the scale (not null).
    ...
    /**
     * Sets the matrix of the <code>Transform</code> to the provided quaternion value. The scale can 
     * therefore be defined using {@link #setScale(float)} or its variants.
     * 
     * @param rotation The rotation matrix if orthonormal, otherwise the rotation
     *                 and the scale (not null).
     * @return         The updated transform for chaining purposes.
    ...
    /**
     * Sets the matrix components of the <code>Transform</code> to the provided euler angles 
     * in radians.
     * 
    ...
    public Transform scale(Vector3f scale) {
        ...
        this.identity = identity && scale.x() == 1.0F && scale.y() == 1.0F && scale.z() == 1.0F;
        this.uniformScale = uniformScale && scale.x() == scale.y() && scale.y() == scale.z();
        return this;
    }
    ...
    /**
     * Return the transformation matrix of the <code>Transform</code>, which is used inside a 
     * {@link ShaderProgram} to compute the correct position, rotation and scale.
     * 
     * @param store The storing matrix, or null to use the internal store of the transform.
    ...
    /**
     * Populates the given {@link FloatBuffer} with the data from the <code>Transform</code> in column major order.
     * <p>
    ...
    public Transform worldTransform(ReadableTransform parent, Transform store) {
        ...
        tmp.reuse();
        var matrixA = isRotationMatrix() ? rotation.multiplyDiagonalPost(scale, tmp) : rotation;
     ...
     * <p>
     * The translation: {0,0,0}. The rotation: {0,0,0,1}. The scale: {1,1,1}.
     * 
    ...
    /**
     * Update the <code>Transform</code> state defined by the boolean variables. The  method should be 
     * called every time the transform internal state has changed and can't be easily determined.
     * 
     * @param rotationMatrixGuaranteed Whether the transform matrix is guaranteed to be rotation-only, 
     *                                 meaning it's orthonormal. This is used to avoid unnecessary and tedius
     *                                 checking in depth checking.
     * 
     * @see #isIdentity()
    ...
    private void update(boolean rotationMatrixGuaranteed) {
        this.identity = translation.equals(Vector3f.ZERO) && rotation.isIdentity() && scale.equals(Vector3f.ONE);
        if (identity) {
            rotationMatrix = true;
            uniformScale = true;
        } else {
    ...
    /**
     * Sets the <code>Transform</code> to the {@link #identity()}, before retrieving
     * it from a pool.
     */
    @Override
    public void reuse() {
    ...
    /**
     * Sets the <code>Transform</code> to the {@link #identity()}, before storing it
     * into a pool.
     */
    @Override
    public void free() {
    ...
    /**
     * Compare this transform with the provided <code>Transform</code>. It will
     * first compare the translation, then the rotation and finally the scale.
     * 
     * @param other The other transform to compare with (not null).
     * @return      0 &rarr; the 2 transforms are equal, negative &rarr; this transform
     *              comes before the other, negative &rarr; this transform comes after
     *              the other.
     */
    @Override
    ...
    public boolean equals(Object o) {
        ...
        return translation.equals(other.getTranslation()) && rotation.equals(other.getRotation())
