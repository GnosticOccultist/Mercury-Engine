...
public class Mesh {
    ...
    /**
     * Instantiates a new <code>Mesh</code> with no {@link VertexBuffer} set. The
     * mode is set by default to {@link Mode#TRIANGLES}.
     * <p>
     * To setup a buffer for this mesh, use
     * {@link #setupBuffer(VertexBufferType, Usage, Buffer)}.
     * 
    ...
    /**
     * Return whether the <code>Mesh</code> is dirty meaning at least one of its
     * {@link VertexBuffer} has unuploaded changes to it.
     * 
    ...
    /**
     * Binds the <code>Mesh</code> to be used by the <code>OpenGL</code> context, by
     * binding its {@link VertexArray}.
     * 
    ...
    /**
     * Setup a {@link VertexBuffer} for the specified array of indices for the
     * <code>Mesh</code>.
     * <p>
     * <b>Only one index buffer can be set</b>, but don't worry this method
     * automatically update the stored data for the buffer type if it's already set.
     * <p>
     * If you want to use the <code>VertexBuffer</code>, you need to upload it to
     * the GPU with the OpenGL context using {@link #upload()}. Note that this
     * function will upload all the buffers already setup on this <code>Mesh</code>.
     * 
    ...
    /**
     * Setup the {@link VertexBuffer} for the specified type and usage and store
     * into it the given buffer containing vertex data.
     * <p>
     * <b>Only one buffer can be set for each {@link VertexBufferType type}</b>, but
     * don't worry this method automatically update the stored data for the buffer
     * type if it's already set.
     * <p>
     * If you want to use the <code>VertexBuffer</code>, you need to upload it to
     * the GPU with the OpenGL context using {@link #upload()}. Note that this
     * function will upload all the buffers already setup on this <code>Mesh</code>.
     ...
     * @param usage       The usage for the buffer (how often it will be updated).
     * @param data        The buffer containing the vertex data.
     * @param updateCount Whether to update the count of vertices based on the position or 
     *                    interleaved buffer.
     */
    ...
    /**
     * Setup the {@link VertexBuffer} for the specified type and usage and store
     * into it the given array of float values.
     * <p>
     * <b>Only one buffer can be set for each {@link VertexBufferType type}</b>, but
     * don't worry this method automatically update the stored data for the buffer
     * type if it's already set.
    ...
    /**
     * Setup the {@link VertexBuffer} for the specified type and usage and store
     * into it the given array of integer values.
     * <p>
     * <b>Only one buffer can be set for each {@link VertexBufferType type}</b>, but
     * don't worry this method automatically update the stored data for the buffer
     * type if it's already set.
    ...
    /**
     * Upload the <code>Mesh</code> to the GPU using the OpenGL context. It means
     * each correctly setup {@link VertexBuffer} will be uploaded and defined with
     * an attribute pointer to use in a {@link ShaderProgram}. After invoking this
     * function, the <code>Mesh</code> can be properly rendered onto the screen
     * using the currently bound shader (if any).
     * <p>
     * If any buffers happen to be modified, you need to call this function again to
     * update the stored vertex values.
     */
    @OpenGLCall
    public void upload() {
        vao.upload();
    ...
    /**
     * Cleanup the <code>Mesh</code> once it isn't needed anymore from the GPU and
     * the OpenGL context.
     * <p>
    ...
    /**
     * Return whether the <code>Mesh</code> as an indices {@link VertexBuffer}
     * present.
     * <p>
     * Note that it only check its presence and not its content or if it had been
     * correctly uploaded to the GPU, this must be guaranteed by the user of the
     * mesh.
     * 
    ...
    /**
     * Return the {@link VertexBuffer} from the {@link VertexBufferType type}, or
    ...
    /**
     * Return the {@link VertexBuffer} using the provided key, or null if it isn't
     * present.
     * 
    ...
    /**
     * Return the {@link Format} of the index {@link VertexBuffer} of the
     * <code>Mesh</code>, or null if it has no indices buffer defined.
     * 
     * @return The vertex buffer corresponding to the specified type.
     */
    ...
    /**
     * Return the number of vertices defined in the <code>Mesh</code> using the
     * indices <code>VertexBuffer</code> or the position <code>VertexBuffer</code>.
     * <p>
    ...
    private void updateVertexCount() {
        ...
        if (buffer == null) {
            throw new NoSuchElementException(
                    "Can't update vertices count " + "without position or interleaved buffer!");
        }
     ...
     * 
     * @param mode The primitive mode (not null).
     */
    ...
    /**
     * Return the number of instance to drawn for the <code>Mesh</code>. If the value is 
     * greater than one, then instanced rendering will be used to draw the mesh.
     * 
    ...
    /**
     * Sets the number of instance to drawn for the <code>Mesh</code>. If the value is 
     * greater than one, then instanced rendering will be used to draw the mesh.
     * 
     * @param instanceCount The count of instances to draw for the mesh (default&rarr;1).
     */
    ...
    /**
     * <code>Mode</code> specifies what kind of OpenGL primitives to render for this {@link Mesh}.
     */
    public enum Mode {
        /**
        ...
        LINES(),
        /**
         * A primitive is a segment, the first one defined with 2 vertices but the next
         * vertices are combined with the previous one to form the other lines.
         */
        LINE_STRIP(),
        /**
         * A primitive is a segment defined with 2 vertices excepting the last one which
         * is connected with the first one to form the final line.
         */
        LINE_LOOP(),
        /**
        ...
        TRIANGLES(),
        /**
         * A primitive is a triangle, the first one defined with 3 vertices but the next
         * vertices are combined with the two previous one to form the other triangles.
         */
        TRIANGLE_STRIP(),
        /**
         * A primitive is a triangle, the first one defined with 3 vertices but the next
         * two vertices are combined with the first one to form the other triangles.
         */
        TRIANGLE_FAN();
    }

    /**
     * Return the equivalent integer value of OpenGL mode for the enumeration
     * {@link Mode}.
     * 
