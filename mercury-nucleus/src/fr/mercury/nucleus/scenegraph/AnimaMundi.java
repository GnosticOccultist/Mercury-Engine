...
public abstract class AnimaMundi {
    ...
    /**
     * An implementation of a visitor to update the transform of a hierarchy of
     * anima-mundi.
     */
    ...
    /**
     * An implementation of a visitor to update the render states of a hierarchy of
     * anima-mundi.
     */
    ...
    /**
     * The environment mode describing how environmental elements should be passed
     * through the scene-graph.
     */
    protected EnvironmentMode envMode = EnvironmentMode.LOCAL_PRIORITY;
    /**
     * The array of environmental elements which are locally present on this
     * anima-mundi.
     */
    protected final Array<EnvironmentElement> envElements = Array.ofType(EnvironmentElement.class);
    /**
     * The accumulated dirty marks by the anima-mundi. At instantiation it will
     * contain {@link DirtyType#TRANSFORM}.
     */
    ...
    /**
     * Instantiates a new <code>AnimaMundi</code> with the {@link Transform} set to
     * identity values.
     */
    ...
    /**
     * Instantiates a new <code>AnimaMundi</code> with the {@link Transform} set to
     * identity values and the given name.
     * 
    ...
    /**
     * Updates the children of the <code>AnimaMundi</code>. The method should be
     * implemented for implementation such as {@link NucleusMundi} to cycle through
     * its children and update their state.
     * 
     * @param timer The timer used by the application (not null).
     */
    ...
    /**
     * Update the world {@link Transform} by combining the local transform of this
     * <code>AnimaMundi</code> with the world one of its parent.
     * <p>
     * If the anima-mundi is {@link #isOrphan() orphan} it will just set the local
     * transform as the world transform.
     */
    protected void updateWorldTransform() {
        logger.debug("Update world transform for " + name);
        if (parent != null) {
            if (parent.isDirty(DirtyType.TRANSFORM)) {
                throw new IllegalStateException("Invalid update of the world transform for " + toString() + ", parent "
                        + parent.toString() + " is dirty!");
            }
    ...
    /**
     * Propagates the {@link DirtyType} down the scenegraph starting from the
     * <code>AnimaMundi</code>. If an implementation of this class handles children,
     * it must propagate the dirty marks to its descendants.
     * 
    ...
    /**
     * Return the local {@link Transform} used by the <code>AnimaMundi</code>. This
     * transform is relative to the parent of this anima-mundi.
     * 
    ...
    /**
     * Return the world {@link Transform} used by the <code>AnimaMundi</code>. This transform is 
     * relative to the whole scene-graph.
     * 
     * @return The world transform of the anima-mundi.
     * @throws IllegalStateException Thrown if the world transform is dirty and should be first refresh.
     * 
    ...
    /**
     * Sets the transform of this <code>AnimaMundi</code> to the provided
     * {@link Transform} in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
    ...
    /**
     * Sets the translation of this <code>AnimaMundi</code> to the provided
     * {@link Vector3f} in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
     * @param translation The translation vector to set (not null).
     * @return            The changed anima-mundi.
     */
    ...
    /**
     * Sets the translation of this <code>AnimaMundi</code> to the provided
     * components in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
    ...
    /**
     * Translates the <code>AnimaMundi</code> by the provided {@link Vector3f} in
     * the local coordinate space. Note that the {@link #getWorldTransform() world
     * transform} won't be updated until {@link #updateGeometricState()} has been
     * called.
     * 
    ...
    /**
     * Translates the <code>AnimaMundi</code> by the provided components in the
     * local coordinate space. Note that the {@link #getWorldTransform() world
     * transform} won't be updated until {@link #updateGeometricState()} has been
     * called.
     * 
     * @param x The X-axis component of the translation.
    ...
    /**
     * Sets the rotation of this <code>AnimaMundi</code> to the provided
     * {@link Quaternion} in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
     * @param translation The rotation quaternion to set (not null).
     * @return            The changed anima-mundi.
     */
    ...
    /**
     * Sets the rotation of this <code>AnimaMundi</code> to the provided
     * {@link Matrix3f} in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
     * @param translation The rotation matrix to set (not null).
     * @return            The changed anima-mundi.
     */
    ...
    /**
     * Sets the rotation of this <code>AnimaMundi</code> to the provided components
     * in the local coordinate space, the W-component is leaved untouched. Note that
     * the {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
    ...
    /**
     * Rotates the <code>AnimaMundi</code> by the provided {@link Quaternion} in the
     * local coordinate space. Note that the {@link #getWorldTransform() world
     * transform} won't be updated until {@link #updateGeometricState()} has been
     * called.
     * 
     * @param rotation The rotation quaternion (not null).
     * @return         The changed anima-mundi.
     */
    ...
    /**
     * Rotates the <code>AnimaMundi</code> by the provided x, y and z angles in
     * radians in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
    ...
    /**
     * Sets the scale of this <code>AnimaMundi</code> to the provided
     * {@link Vector3f} in the local coordinate space. Note that the
     * {@link #getWorldTransform() world transform} won't be updated until
     * {@link #updateGeometricState()} has been called.
     * 
     * @param translation The scaling vector to set (not null).
     * @return            The changed anima-mundi.
     */
    ...
    /**
     * Scales the <code>AnimaMundi</code> by the provided {@link Vector3f} in the
     * local coordinate space. Note that the {@link #getWorldTransform() world
     * transform} won't be updated until {@link #updateGeometricState()} has been
     * called.
     * 
     * @param scale The scaling vector (not null).
     * @return      The changed anima-mundi.
     */
     ...
     * coordinate space. Note that the {@link #getWorldTransform() world transform}
    ...
    /**
     * Sets the parent of the <code>AnimaMundi</code>. The method should be called internally by 
     * the {@link NucleusMundi} implementation.
     * 
    ...
    /**
     * Return whether the <code>AnimaMundi</code> is orphan, meaning it has no parent.
     * 
    ...
    /**
     * Return whether the <code>AnimaMundi</code> has the provided {@link NucleusMundi} for 
    ...
    /**
     * Return the used {@link BucketType} for the rendering of this <code>AnimaMundi</code>. If the <code>checkLegacy</code> 
     * is set to true, it will search the parent's bucket type until it founds something different than {@link BucketType#LEGACY} 
     * or until the ancestor is orphan.
     * 
    ...
    public BucketType getBucket(boolean checkLegacy) {
        ...
        // If no other bucket type is specified in the hierarchy, default to the OPAQUE
        // bucket.
    ...
    /**
     * Return the {@link RenderLayer} on which the <code>AnimaMundi</code> is present. It will try to find the 
     * ancestor's render layer if its type is {@link RenderLayer#LEGACY}.
     * 
    ...
    /**
     * Return the {@link RenderLayer} on which the <code>AnimaMundi</code> is present. If the <code>checkLegacy</code> 
     * is set to true, it will search the parent's render layer until it founds something different than {@link RenderLayer#LEGACY} 
     * or until the ancestor is orphan.
     * 
    ...
    public RenderLayer getRenderLayer(boolean checkLegacy) {
        ...
        // If no other render layer is specified in the hierarchy, default to the
        // DEFAULT layer.
     ...
     * 
     * @throws IllegalArgumentException Throw if the element added is already
     *                                  present on this anima-mundi, and only one
     *                                  can be present.
     */
    public void addEnvironmentElement(EnvironmentElement element) {
        ...
        if (element.isSingleton() && containsLocal(element.name())) {
            throw new IllegalArgumentException(
                    "The provided environment element is already defined for '" + getName() + "' !");
        }
    ...
    /**
     * Return whether an {@link EnvironmentElement} matching the provided name is
     * present locally on this <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Return an {@link EnvironmentElement} matching the provided name by searching
     * locally and through upper-hierarchy depending on the {@link EnvironmentMode}
     * of the <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Return an {@link Optional} value containing the {@link EnvironmentElement}
     * present locally on the <code>AnimaMundi</code> matching the provided name.
     * <p>
     * If no such element exists it will return an empty optional value.
     * 
     * @param name The name of the enviromnent element to get.
     * @return     An optional value containing the environment element matching the
     *             name, or empty if no such element exists.
     * 
     * @see #getLocalEnvironmentElement(String)
     */
    public Optional<EnvironmentElement> getLocalEnvironmentElementOpt(String name) {
        var optionalElement = envElements.stream().filter(element -> element.name().equalsIgnoreCase(name)).findFirst();

        return optionalElement;
    }

    /**
     * Return an {@link EnvironmentElement} present locally on the <code>AnimaMundi</code> matching 
     * the provided name.
     * 
     * @param name The name of the enviromnent element to get.
     * @return     The environment element matching the name.
     ...
     * 
     * @throws NoSuchElementException Thrown if no element present on the anima-mundi is matching the provided name.
     */
    ...
    /**
     * Return a readable-only array of the {@link EnvironmentElement} locally
     * present on the <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Return the local {@link RenderState} corresponding to the provided {@link Type} locally 
     * present on the <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Collects the {@link RenderState} defined in the scenegraph to the given
     * table, to be later applied to the <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Applies all previously collected {@link RenderState} in the scenegraph inside
     * the given table to the <code>AnimaMundi</code>.
     * 
    ...
    /**
     * Visit the <code>AnimaMundi</code> with the given {@link Visitor} and
     * {@link VisitType}. The visit type defines in which order the concerned mundi
     * should be visited.
     * 
