 ...
 * <p>
 * It is in charge of sorting these animae based on the provided {@link Comparator}, to increase 
 * performance and quality of the rendering process by limiting, for example, <code>OpenGL</code> 
 * state changes, redrawn pixel or drawing background object prior to a transparent one.
 * 
...
public class RenderBucket {
    ...
    /**
     * Instantiates a new <code>RenderBucket</code> with an initial size of
     * {@link #INITIAL_SIZE} and the given {@link Camera}.
     * 
    ...
    /**
     * Instantiates a new <code>RenderBucket</code> with the specified initial size
     * and {@link Camera}.
     * 
    ...
    /**
     * Instantiates a new <code>RenderBucket</code> with an initial size of
     * {@link #INITIAL_SIZE}, the {@link Camera} and {@link Comparator} to use to
     * sort the {@link AnimaMundi} in the bucket. Usually the comparator uses the
     * distance of the object to render from the camera.
     * 
     * @param camera     The camera used to compute distances (not null).
     * @param comparator The comparator to sort the bucket, usually a distance
     *                   factor (not null).
     */
    ...
    /**
     * Instantiates a new <code>RenderBucket</code> with the specified initial size,
     * the {@link Camera} and {@link Comparator} to use to sort the
     * {@link AnimaMundi} in the bucket. Usually the comparator uses the distance of
     * the object to render from the camera.
     * 
     * @param size       The initial size of the bucket (&ge;0).
     * @param camera     The camera used to compute distances (not null).
     * @param comparator The comparator to sort the bucket, usually a distance
     ...
     * The queue distance field of the anima is set to
     * {@link Double#NEGATIVE_INFINITY} to be recomputed. <br>
     * Note that if the bucket is already filled entirely, it will automatically
     * double its size.
     * 
     ...
     * <code>shellsort</code> sorter. Note that the comparator cannot be null.
     */
    public void sort() {
    ...
    /**
     * Render the <code>RenderBucket</code> by calling
     * {@link AbstractRenderer#render(PhysicaMundi)} for each {@link PhysicaMundi},
     * and resetting the queue distance field.
     * 
    ...
    /**
     * Compute the distance between the provided {@link AnimaMundi} and the
     * registered {@link Camera}. Note that the camera cannot be null.
     * 
    ...
    /**
     * Merges the <code>RenderBucket</code>'s content with the specified bucket's
     * one, by first adding the content of this bucket and then the content of the
     * other bucket.
     * <p>
    ...
    /**
     * Flushes the <code>RenderBucket</code> and all the contained
     * {@link AnimaMundi}, if any. The size of the bucket is reset to 0.
     */
    public void flush() {
    ...
    /**
     * Sets the {@link Camera} used to sort the {@link AnimaMundi} to render the
     * <code>RenderBucket</code>.
     * 
    ...
    /**
     * Sets the {@link Comparator} used to sort the {@link AnimaMundi} to render the
     * <code>RenderBucket</code>.
     * 
