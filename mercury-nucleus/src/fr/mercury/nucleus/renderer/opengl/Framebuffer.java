...
public class Framebuffer extends GLObject {
    ...
    /**
     * The array of currently bound GL framebuffers to the context, normally one for
     * each target.
     */
    ...
    /**
     * Instantiates a new <code>Framebuffer</code> to use for the specified
     * {@link FramebufferTarget} in the OpenGL context.
     * 
    ...
    /**
     * Binds the <code>Framebuffer</code> to the OpenGL context, allowing it to be
     * used or updated.
     */
    @OpenGLCall
    public void bind() {
     ...
     * <p>
     * This methods is mainly used for proper cleaning of the <code>OpenGL</code> context or to avoid errors 
     * of misbindings, because it doesn't need to be called before binding a framebuffer.
     * <p>
     * If you want to specify which target type to unbound from the context use {@link #unbind(FramebufferTarget)} 
     * instead.
     * 
    ...
    /**
     * Unbinds the currently bound <code>Framebuffer</code> on the specified {@link FramebufferTarget} from 
     * the OpenGL context.
     * <p>
     * This methods is mainly used for proper cleaning of the <code>OpenGL</code> context or to avoid errors 
     * of misbindings, because it doesn't need to be called before binding a framebuffer.
     ...
     * 
     * @return The deleting action to delete the framebuffer from the OpenGL
     *         context.
     */
    @Override
    @OpenGLCall
     ...
     * commands with a {@link Framebuffer} inside the OpenGL context. It means that a context can have one framebuffer
     * used only for reading purposes and another one for writing bound at the same time.
     * 
    ...
    public enum FramebufferTarget {
        ...
        READ,
        /**
         * Only perform writing commands to the framebuffer such as all rendering
         * commands.
         */
        DRAW,
        /**
        ...
        /**
         * Return the equivalent OpenGL framebuffer target. Either
         * {@link GL30#GL_FRAMEBUFFER}, {@link GL30#GL_READ_FRAMEBUFFER} or
         * {@link GL30#GL_DRAW_FRAMEBUFFER}.
         * 
