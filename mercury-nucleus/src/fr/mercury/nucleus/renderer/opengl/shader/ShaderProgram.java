...
public final class ShaderProgram extends GLObject {
     ...
     * <p>
     * Please use {@link #attachSource(ShaderSource)} to add a
     * <code>ShaderSource</code> and {@link #upload()} to upload the program to the
     * GPU.
     */
    ...
    public void upload() {
        ...
        if (needsUpdate()) {
            ...
            var linkStatus = GL20.glGetProgrami(id, GL20.GL_LINK_STATUS);
            if (linkStatus == GL11.GL_FALSE) {
                var errorSource = GL20.glGetProgramInfoLog(id, 1024);
    ...
    /**
     * Install this <code>ShaderProgram</code> as part of the current rendering
     * state of <code>OpenGL</code>. It keeps track of the bounded program, to
     * prevent useless call of this function.
     * 
     ...
     * 
     * @param source The source to attach (not null).
     ...
     * 
     * @param sources The sources to attach (not null).
    ...
    /**
     * Detach the provided {@link ShaderSource} from the <code>ShaderProgram</code>,
     * if it was previously attached to it. Note that if the source or the program
     * isn't yet created, it will not continue and leave a warning message.
     * 
     ...
     * name, type and value. If a uniform with the name already exists, it will
     * simply set its type and value without instantiating a new one.
     * 
    ...
    /**
     * Registers all {@link Uniform} present on the provided object into the
     * <code>ShaderProgram</code>. This method identifies getter methods of uniform
     * value using the {@link UniformField} annotation, which must specify the name
     * and {@link UniformType} of the uniform.
     * <p>
     * A uniform getter method is declared as valid by the method if:
     * <ul>
     * <li>The method is annotated with the <code>UniformField</code>
     * annotation.</li>
     * <li>The method is declared as public and accessible.</li>
     * <li>The method doesn't need any parameters.</li>
     * </ul>
     * 
     * Note that the method, however, doesn't need to start with the 'get...'
     * syntax, as opposed to <code>Java</code> getters.
     * 
    ...
    /**
     * Return the registered {@link Uniform} with the provided name, or null if it
     * doesn't exist.
     * 
    ...
    /**
     * Return whether the <code>ShaderProgram</code> needs to be relinked, to take
     * into account the latest changes.
     * 
