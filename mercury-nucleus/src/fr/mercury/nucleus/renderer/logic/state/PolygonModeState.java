...
public final class PolygonModeState extends RenderState {
    ...
    /**
     * Instantiates a new disabled <code>PolygonModeState</code> with all
     * {@link Face} type set to the {@link PolygonMode#FILL} polygon mode, which
     * means the interior of each polygon will be rasterized as filled.
     * 
     * @see #reset()
     */
    ...
    /**
     * Enable the <code>PolygonModeState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will enable a special rasterization of polygons of
     * the specified {@link Face} using the set {@link PolygonMode}.
     * 
    ...
    /**
     * Disable the <code>PolygonModeState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will disable the special rasterization of polygons
     * of the specified {@link Face} using the set {@link PolygonMode}. It means
     * that the interior of each polygon will be filled.
     * <p>
    ...
    /**
     * Return the {@link PolygonMode} used by the <code>PolygonModeState</code> for
     * the provided {@link Face} type to determine how each polygons are rasterized.
     * 
     * @param face The face type to get the polygon mode (not null).
    ...
    /**
     * Sets the {@link PolygonMode} to use by the <code>PolygonModeState</code> for
     * the rasterization of the polygons of the given {@link Face} type.
     * 
    ...
    public PolygonModeState setPolygonMode(Face face, PolygonMode mode) {
        ...
        // Only update the polygon modes if the requested mode isn't already set for the
        // face.
        if (needsUpdate) {
    ...
    /**
     * Resets the <code>PolygonModeState</code> to its default state, meaning its
     * disabled with all {@link Face} type set to {@link PolygonMode#FILL}.
     */
    @Override
    public void reset() {
        this.enabled = false;
        // Make sure the EnumMap exists cause it may not if the user is chaining
    ...
    /**
     * <code>PolygonMode</code> is an enumeration of all possible modes for drawing
     * a polygon by filling it, drawing its lines or points.
     * 
     * @author GnosticOccultist
     */
