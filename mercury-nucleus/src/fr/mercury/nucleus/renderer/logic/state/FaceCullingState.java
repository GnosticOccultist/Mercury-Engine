...
public final class FaceCullingState extends RenderState {
    ...
    /**
     * The winding order to determine which side is considered to be the front, only
     * used when drawing triangles (default&rarr;COUNTER_CLOCKWISE).
     */
    private WindingOrder windingOrder = WindingOrder.COUNTER_CLOCKWISE;

    /**
     * Instantiates a new disabled <code>FaceCullingState</code> with the culling
     * face set to {@link Face#BACK} and the winding order to
     * {@link WindingOrder#COUNTER_CLOCKWISE}.
     * 
     * @see #reset()
     */
    ...
    /**
     * Enable the <code>FaceCullingState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will enable the culling of the specified
     * {@link Face} using the set {@link WindingOrder}.
     * 
    ...
    /**
     * Disable the <code>FaceCullingState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will disable the culling of the specified
     * {@link Face} using the set {@link WindingOrder}.
     * <p>
    ...
    /**
     * Return the {@link WindingOrder} used by the <code>FaceCullingState</code> to
     * determine which side is considered to be the front (either clockwise or
     * counter-clockwise). It is only used when the primitive used for drawing are
     * triangles.
     * 
    ...
    /**
     * Return the {@link Face} to cull by the <code>FaceCullingState</code> when
     * enabled (either front face, back face or both). It is only used when the
     * primitive used for drawing are triangles.
     * 
    ...
    /**
     * Sets the {@link Face} to cull by the <code>FaceCullingState</code> when
     * enabled (either front face, back face or both). It is only used when the
     * primitive used for drawing are triangles.
     * 
    ...
    /**
     * Sets the {@link WindingOrder} used by the <code>FaceCullingState</code> to
     * determine which side is considered to be the front (either clockwise or
     * counter-clockwise). It is only used when the primitive used for drawing are
     * triangles.
     * 
    ...
    /**
     * Resets the <code>FaceCullingState</code> to its default state, meaning its
     * disabled with the face to cull as {@link Face#BACK} and the winding order as
     * {@link WindingOrder#COUNTER_CLOCKWISE}.
     */
    @Override
    public void reset() {
    ...
    /**
     * <code>WindingOrder</code> is an enumeration of all possible method to
     * determine which side is considered to be the front in the
     * {@link FaceCullingState}.
     * 
     * @author GnosticOccultist
     */
    public enum WindingOrder {
        /**
         * The clockwise winding order will select the 3 vertices of the triangle by
         * rotating clockwise around the triangle's center.
         */
        CLOCKWISE,
        /**
         * The counter-clockwise winding order will select the 3 vertices of the
         * triangle by rotating counter-clockwise around the triangle's center.
         */
        COUNTER_CLOCKWISE;
    }
}
