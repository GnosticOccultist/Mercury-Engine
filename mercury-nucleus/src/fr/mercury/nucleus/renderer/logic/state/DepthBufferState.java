...
public final class DepthBufferState extends RenderState {
    ...
    /**
     * Instantiates a new disabled <code>DepthBufferState</code> with writable set
     * to true and function to {@link DepthFunction#LESS}.
     * 
     * @see #reset()
     */
    ...
    /**
     * Enable the <code>DepthBufferState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will enable the depth testing used when rendering
     * fragments using the set {@link DepthFunction}.
     * 
    ...
    /**
     * Disable the <code>DepthBufferState</code>. When applied as a render state for
     * an {@link AnimaMundi}, it will disable the depth testing used when rendering
     * fragments.
     * <p>
    ...
    /**
     * Return whether the <code>DepthBufferState</code> is writable, meaning the
     * fragments z values will be stored in the depth buffer if they pass the depth
     * test exprimed with the {@link DepthFunction}.
     * <p>
     * If the <code>DepthBufferState</code> isn't writable then the depth buffer
     * will be readable only but this has an effect only if {@link #enable()}.
     * 
    ...
    /**
     * Return the {@link DepthFunction} used by the <code>DepthBufferState</code> to
     * determine if a fragment passes the depth test and can be written into the
     * depth buffer.
     * 
    ...
    /**
     * Set the <code>DepthBufferState</code> as being writable, meaning the
     * fragments z values will be stored in the depth buffer if they pass the depth
     * test exprimed with the {@link DepthFunction}.
     * <p>
     * If the <code>DepthBufferState</code> isn't writable then the depth buffer
    ...
    /**
     * Set the <code>DepthBufferState</code> as being masked or read-only, meaning
     * the fragments z values will be tested and discarded accordingly to the
     * {@link DepthFunction} but not written to the depth buffer.
     * <p>
     * If the <code>DepthBufferState</code> isn't writable then the depth buffer
     * will be readable only but this has an effect only if {@link #enable()}.
     * 
    ...
    /**
     * Sets the {@link DepthFunction} used by the <code>DepthBufferState</code> to
     * determine if a fragment passes the depth test and can be written into the
     * depth buffer.
     * 
    ...
    /**
     * <code>DepthFunction</code> is an enumeration of all possible depth comparison
    ...
    public enum DepthFunction {
        ...
        LESS,
        /**
         * Passes if the fragment's depth value is less than or equal to the stored
         * depth value.
         */
        LESS_OR_EQUAL,
        /**
        ...
        GREATER,
        /**
         * Passes if the fragment's depth value is greater than or equal to the stored
         * depth value.
         */
        GREATER_OR_EQUAL;
    }
